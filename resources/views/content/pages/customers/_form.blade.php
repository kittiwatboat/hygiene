...
<div class="row g-4">
  ...
  <div class="col-md-6">
    ...
    <input
      type="text"
      name="line_id"
      value="{{ old('line_id', $customer->line_id ?? '') }}"
  ...
  <div class="col-md-6">
    <label class="form-label">
      ประเภทสมาชิก <span class="text-danger">*</span>
    </label>

    @php
      $selectedMemberType = old(
          'member_type',
          $customer->member_type ?? 'new_member'
      );
    @endphp
    ...
    <select
      ...
      required
    >
      @foreach (\App\Models\Customer::MEMBER_TYPE_OPTIONS as $value => $label)
        <option
          value="{{ $value }}"
          {{ $selectedMemberType === $value ? 'selected' : '' }}
        >
          {{ $label }}
        </option>
      @endforeach
    ...
    <div class="col-md-6">
      ...
      <div class="input-group">
        <input
          type="text"
          class="form-control"
          value="{{ number_format((int) ($customer->points_balance ?? 0)) }}"
          readonly
        >
      ...
      <div class="form-text">
        แต้มสะสมเป็นข้อมูลจากระบบ และไม่สามารถแก้ไขจากหน้าจัดการสมาชิกได้
      </div>
    </div>
  @endisset
