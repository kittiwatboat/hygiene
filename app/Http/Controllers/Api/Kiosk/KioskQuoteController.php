...
class KioskQuoteController extends Controller
{
    public function calculate(Request $request): JsonResponse
    {
        try {
            ...
            foreach ($validated['items'] as $index => $input) {
                ...
                if (!$tank) {
                    throw ValidationException::withMessages([
                        "items.$index.tank_id" => 'Tank ที่เลือกไม่อยู่ในเครื่องนี้',
                    ]);
                }
                ...
                if (!$product || !(bool) $product->is_active) {
                    throw ValidationException::withMessages([
                        "items.$index.tank_id" => 'สินค้าใน Tank นี้ไม่สามารถใช้งานได้',
                    ]);
                }
                ...
                $items->push([
                    ...
                    'selection' => [
                        ...
                        'normal_line_total' => round($normalLineTotal, 2),
            ...
            return response()->json([
                'success' => true,
                'message' => 'ตรวจสอบรายการสินค้าที่เลือกสำเร็จ',
                'data' => [
                    'machine' => [
                        'id' => $machine->id,
                        'name' => $machine->name,
                    ...
                    'items' => $items->values(),
                    'summary' => [
                        'total_items' => (int) $items->sum(
                            fn ($item) => $item['selection']['quantity']
                        ),
            ...
            return response()->json([
                'success' => false,
                'message' => 'ไม่สามารถตรวจสอบรายการสินค้าได้',
            ], 500);
        }
    }
}
